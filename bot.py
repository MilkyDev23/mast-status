...
class Client(discord.Client):
    ...
    async def on_message(self, message):
        ...
        elif lower_message.startswith("!tuvs") and message.author.guild_permissions.administrator:
            if self.show_uptime:
                self.show_uptime = False
                await message.channel.send("Online time is no longer displayed :urbanx:")
            else:
                self.show_uptime = True
                await message.channel.send("Online time is now displayed :urbancheckmark:")
    ...
    def create_status_template() -> discord.Embed:
        ...
        # )
        embed.description = "> **SERVER STATUS**\n```✅ Online ```"
    ...
    def add_fivem_status_to_status_message(self, embed):
        if self.show_cfx_status:
            embed.add_field(
                name="\u200b",
                value="**[Click to Connect](https://cfx.re/join/e43jl3)**",
                inline=False,
            )
        # if self.show_down_detector_status:
        #     embed.add_field(
    ...
    def create_status_online(self) -> discord.Embed:
        ...
        embed.title = "Urban Escape Status"
        embed.url = "https://cfx.re/join/e43jl3"
        embed.colour = 0x206694
        embed.set_thumbnail(url="https://media.discordapp.net/attachments/1034565689727533133/1130202591339483166/image.png")
        embed.add_field(
            name="> **PLAYERS**",
        ...
        # embed.add_field(
        #     name="> **CFX STATUS**",
        #     value="```" + {self.add_dot_to_fivem_status(self.cfx_status)} + "```",
        #     inline=True,
        # )
        embed.add_field(
            name="> **CFX STATUS**",
            value="```"f"{self.cfx_status}""```",
            inline=True,
        )
        # add uptime field
    ...
    def create_status_offline(self) -> discord.Embed:
        embed = self.create_status_template()
        embed.title = "Urban Escape Status"
        embed.description = "> **SERVER STATUS**\n```❌ Offline```\nPlease check the [CFX Status](https://status.cfx.re/) to see if it's on our end, if everything is operational please wait whilst our development team fix the issue.\n\n**Current CFX Status**\n> "f"{self.cfx_status}"""
        embed.url = "https://cfx.re/join/e43jl3"
        ...
        if fiveMServer.get_downtime_seconds() > 60:
            ...
            embed.add_field(
                name="\u200b",
                value="> Please check the CFX Status to see if it's on our end, if everything is operational please wait whilst our development team fix the issue.",
                inline=False,
            )
        self.add_fivem_status_to_status_message(embed)
        return embed
